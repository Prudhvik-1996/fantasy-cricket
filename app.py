...
def homepage():
    print(matches)
    return render_template('homepage.html', message="", matches=matches)
...
def match(id):
    try:
        if session['logged_in']:
            ...
            return render_template('match.html',matches=matches, c=c, id=id, team=team, leaderboard=leaderboard)
        else:
            return render_template('login.html', error="")
    except:
        return render_template('login.html', error="")
    ...
    try:
        if session['logged_in']==True:
            if request.method == 'POST':
                ...
                return render_template('selectpowerplayer.html', dreamteam=dream, matchName=matchName)
    except:
        return redirect(url_for('login'))
    return redirect(url_for('login'))
    # return "Go to Hell!!"
